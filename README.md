...
| **TPR** (True Positive Rate) | $\frac{\text{Correctly Pred. "Approved"}}{\text{Historically Approved}}$ | Measures **Opportunity Equity**. A lower TPR suggests the model fails to identify successful candidates, leading to missed opportunities. |
| **FPR** (False Positive Rate) | $\frac{\text{Incorrectly Pred. "Approved"}}{\text{Historically Denied}}$ | Measures **Risk Equity**. A higher FPR indicates unintended leniency or higher risk by approving applications that the historical record rejected. |
